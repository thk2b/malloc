...
static inline t_free_block	*do_coalesce(
	t_area *a, t_free_block *fb, size_t requested_size)
{
	size_t			total;
	t_free_block	*cur;
	void			*end;
...
static inline t_free_block	*do_coalesce_back(
	t_area *a, t_free_block *fb, size_t offset, size_t requested_size)
{
	t_free_block	*new_fb;
...
static inline int			should_coalesce(
	size_t reclaimed_size, size_t requested_size)
{
	return (reclaimed_size >= requested_size / 4);
}

/*
**	Attempt to gather at least requested_size
**		from free blocks surounding fb
**	The rightmost free block of the same list as fb
**		is set in last_encountered_adjacent_fb
**	Returns the extended free block or NULL
**		if not enough space could be gathered
**	When extending, free blocks are removed
**		from their list and destroyed apropriately
**	The returned free block may be before fb in the
**		case where there was free space before fb
**	If rhe resulting block exceeds requested_size, it is split
**	The coalesced block is not added to a free list
*/

t_free_block				*area__coalesce_free_block(
	t_area *a, t_free_block *fb, size_t requested_size,
	t_free_block **last_encountered_adjacent_fb)
{
	...
	if (total_size < requested_size)
		space_after = area__count_free_space_after(a,
			&fb->block, requested_size, last_encountered_adjacent_fb);
